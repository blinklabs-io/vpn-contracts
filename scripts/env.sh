...
# Derive policy ID using blake2b_256(policy + "nullifier")
# Note: Nullifier tokens are now minted under VPN policy directly,
# so this function is only used for Midnight config derivation
...
suffix = b"nullifier"
combined = policy_bytes + suffix
result = hashlib.blake2b(combined, digest_size=32).digest()
print(result.hex())
EOF
}

# Derive Midnight config policy from reference policy
# Used to find the Midnight configuration UTxO
