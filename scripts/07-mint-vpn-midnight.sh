...
# Nullifier tokens are minted under VPN policy (same as VPN tokens)
# No separate nullifier validator needed
...
# Build transaction
# Note: We mint both the VPN token AND the nullifier token under VPN policy
echo ""
...
cardano-cli conway transaction build \
    ...
    --mint "1 $VPN_CS.$TN + 1 $VPN_CS.$NULLIFIER" \
